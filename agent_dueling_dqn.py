...
# Only difference is the Neural network architecture: Decompose the 
# Q function into value and advantage streams
def Neural_Network(lr,n_actions,input_dim):
    Model_input = Input(shape=(input_dim,))
    Dense1 = Dense(256,input_shape=(input_dim,),activation='relu')(Model_input)
    Dense2 = Dense(256,activation='relu')(Dense1)
    Value = Dense(1)(Dense2)
    Value = Lambda(lambda x: K.expand_dims(x[:, 0], -1),output_shape=(n_actions,))(Value)
    Advantage = Dense(n_actions)(Dense2)
    Advantage = Lambda(lambda x: x[:, :]-K.mean(x[:, :], keepdims=True), output_shape=(n_actions,))(Advantage)
    Q_values = Add()([Value,Advantage])
    model = Model(inputs = Model_input, outputs = Q_values)
    model.compile(optimizer=Adam(lr=lr),loss='mse')
    return model
    
    
## Class Dueling DQN Agent 
